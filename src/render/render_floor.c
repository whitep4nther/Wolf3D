...
/*   Updated: 2015/06/01 11:02:33 by ihermell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			render_floor(int from, t_sector *sector, t_render *r, t_env *e)
{
	...
	while (from > r->current_top)
	{
		...
		x = abs((int)floor(r->reference_pos.x + distance * r->ray_cos) % txt->width);
		y = abs((int)floor(r->reference_pos.y + distance * r->ray_sin) % txt->height);
